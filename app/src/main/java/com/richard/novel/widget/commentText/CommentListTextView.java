...
//    public CommentListTextView setmTextSize(int mTextSize) {
...
//    }
    ...
    public void setData (List<CommentInfo> mInfos) {
         ...
         */
        setMovementMethod (LongClickMovementMethod.getInstance ());
        setHighlightColor (Color.TRANSPARENT);
        setText (getCommentString ());
//        setTextSize(TypedValue.COMPLEX_UNIT_SP,getmTextSize());
    ...
    private SpannableStringBuilder getCommentString () {
        ...
        for (int mI = 0; mI < mInfos.size (); mI++) {
            ...
            int end = 0;
            try {
                end = (mInfo.getNickname()).length ();
            }catch (Exception e){
                LogUtil.e("存在空用户名"+mInfo.getUserId());
            }
            ...
            try {
                nickNameLength = mInfo.getNickname ().length();
            }catch (Exception e){
                LogUtil.e("存在空用户名"+mInfo.getUserId());
            }
            if(mInfo.getTonickname () != null && !mInfo.getTonickname ().equals ("")){
                mString.setSpan (new ForegroundColorSpan(mTalkColor),nickNameLength,nickNameLength+mTalkStr.length (), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }else{
                mString.setSpan (new ForegroundColorSpan(mTalkColor),nickNameLength,nickNameLength, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
            ...
            mString.setSpan (new LongClickableSpan() {
                ...
                public void onLongClick(View view) {
                    if(mListener != null){
                        mListener.onCommentItemLongClick(finalMI, mInfos.get (finalMI));
                    }
                }
            ...
            if (mMaxlines!=0 && mI == (mMaxlines - 1)) {
                break;
            }
        }
        if (mMaxlines!=0 && mInfos.size () > mMaxlines) {
    ...
    public interface onCommentListener {
         ...
         * 长按
         */
        public void onCommentItemLongClick(int position, CommentInfo mInfo);
    }
    ...
    public static  class CommentInfo {
        ...
        public CommentInfo setID (final String mID) {
            ID = mID;
            return this;
        }
        ...
        public CommentInfo setUserId(String userId) {
            this.userId = userId;
            return this;
        }
        ...
        public CommentInfo setToUserId(String toUserId) {
