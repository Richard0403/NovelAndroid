...
import com.richard.novel.common.utils.IOUtils;
import com.richard.novel.common.utils.RxUtils;
...
import com.richard.novel.reader.ReadSettingManager;
...
public abstract class PageLoader {
    ...
    // 当前章节列表
    protected List<BookChapterInfo> mChapterList;
    // 书本对象
    ...
    private void initData() {
        ...
        // 初始化参数
        mMarginWidth = ScreenUtils.dpToPxInt(DEFAULT_MARGIN_WIDTH);
        mMarginHeight = ScreenUtils.dpToPxInt(DEFAULT_MARGIN_HEIGHT);
        // 配置文字有关的参数
    ...
    private void setUpTextParams(int textSize) {
        // 文字大小
        mTextSize = textSize;
        mTitleSize = (int) (mTextSize + ScreenUtils.spToPx(EXTRA_TITLE_SIZE));
        // 行间距(大小为字体的一半)
     ...
     * @return
     */
    public List<BookChapterInfo> getChapterCategory() {
        return mChapterList;
    }
    ...
    public void saveRecord() {
        ...
        mBookRecord.setId(mCollBook.getId());
        ...
        //存储到数据库
        BoxStoreHelper.getInstance().put(BookInfo.class, mBookRecord);
    ...
    private void prepareBook() {
        BookInfo bookInfo = BookQuery.queryBook(mCollBook.getId());
        if(bookInfo==null){//本地无存储的bookInfo
            mBookRecord = mCollBook;
        }else{
            mBookRecord = bookInfo;
        }
    ...
    private List<TxtPage> loadPageList(int chapterPos) throws Exception {
        // 获取章节
        BookChapterInfo chapter = mChapterList.get(chapterPos);
        // 判断章节是否存在
     ...
     * 刷新章节列表
     */
    public abstract void refreshChapterList(List<BookChapterInfo> chapterList);
     ...
     * @return
     */
    protected abstract BufferedReader getChapterReader(BookChapterInfo chapter) throws Exception;
     ...
     * @return
     */
    protected abstract boolean hasChapterData(BookChapterInfo chapter);
    ...
    private void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        int tipMarginHeight = ScreenUtils.dpToPxInt(3);
        if (!isUpdate) {
            /****绘制背景****/
        ...
        int polarHeight = ScreenUtils.dpToPxInt(6);
        int polarWidth = ScreenUtils.dpToPxInt(2);
        int border = 1;
        int innerMargin = 1;
        ...
        Rect polar = new Rect(polarLeft, polarTop, visibleRight,
                polarTop + polarHeight - ScreenUtils.dpToPxInt(2));
        ...
        int outFrameBottom = visibleBottom - ScreenUtils.dpToPxInt(2);
     ...
     * @return
     */
    private List<TxtPage> loadPages(BookChapterInfo chapter, BufferedReader br) {
        //生成的页面
    ...
    public interface OnPageChangeListener {
         ...
         */
        void requestChapters(List<BookChapterInfo> requestChapters);
         ...
         */
        void onCategoryFinish(List<BookChapterInfo> chapters);
