...
public class ReadSettingDialog extends Dialog {
    ...
    @BindView(R.id.swt_volume_page)
    Switch swt_volume_page;
...
//    TextView mTvMore;
    ...
    private void initClick() {
        ...
        mCbFontDefault.setOnCheckedChangeListener(
                (buttonView, isChecked) -> {
                    if (isChecked) {
                        int fontSize = ScreenUtils.dpToPxInt(DEFAULT_TEXT_SIZE);
                        mTvFont.setText(fontSize + "");
                        mPageLoader.setTextSize(fontSize);
                    }
                }
        ...
        //背景的点击事件
        mPageStyleAdapter.setOnItemClickListener((adapter, view, position) -> {
                    mPageLoader.setPageStyle(PageStyle.values()[position]);
                }
        );

        swt_volume_page.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if(isChecked){
                ReadSettingManager.setVolumeTurnPage(true);
            }else{
                ReadSettingManager.setVolumeTurnPage(false);
            }
        });
